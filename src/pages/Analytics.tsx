...
import { performanceData, assetPerformance, providers } from "@/data/mockData";
    ...
    <div className="space-y-6 max-w-7xl">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="glass-card p-5 opacity-0 animate-fade-up" style={{ animationDelay: "80ms" }}>
          <h2 className="text-sm font-semibold text-foreground mb-4">PnL Over Time</h2>
          <div className="h-[260px]">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={performanceData}>
                <CartesianGrid strokeDasharray="3 3" stroke="hsl(220 14% 16%)" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="glass-card p-5 opacity-0 animate-fade-up" style={{ animationDelay: "160ms" }}>
          <h2 className="text-sm font-semibold text-foreground mb-4">Performance by Asset</h2>
          <div className="h-[260px]">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={assetPerformance} layout="vertical">
                <CartesianGrid strokeDasharray="3 3" stroke="hsl(220 14% 16%)" horizontal={false} />
                <XAxis type="number" tick={{ fill: "hsl(215 12% 52%)", fontSize: 12 }} axisLine={false} tickLine={false} tickFormatter={(v) => `${v}%`} />
                <YAxis type="category" dataKey="asset" tick={{ fill: "hsl(215 12% 52%)", fontSize: 12 }} axisLine={false} tickLine={false} width={85} />
                <Tooltip contentStyle={tooltipStyle} />
                <Bar dataKey="winRate" radius={[0, 4, 4, 0]} barSize={20}>
                  {assetPerformance.map((_, i) => (
                    <Cell key={i} fill={`hsl(${152 + i * 30} 50% 48%)`} />
                  ))}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
        ...
        <div className="glass-card p-5 opacity-0 animate-fade-up" style={{ animationDelay: "240ms" }}>
          <h2 className="text-sm font-semibold text-foreground mb-4">Performance by Provider</h2>
              ...
              <div key={p.id} className="flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <span className="text-xs text-muted-foreground tabular-nums">{p.winRate}%</span>
                  </div>
